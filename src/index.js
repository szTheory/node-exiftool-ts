...
class ExiftoolProcess extends EventEmitter {
  ...
  open(encoding, options) {
    ...
    return lib.spawn(this._bin, _options).then((exiftoolProcess) => {
      ...
      if (!lib.isReadable(this._process.stdout)) {
        ...
        throw new Error(
          "Process was not spawned with a readable stdout, check stdio options."
        );
      }
      if (!lib.isWritable(this._process.stdin)) {
        lib.killProcess(this._process);
        throw new Error(
          "Process was not spawned with a writable stdin, check stdio options."
        );
      }
      ...
      this._stdoutResolveWs = beginReady.setupResolveWriteStreamPipe(
        this._process.stdout
      );
      this._stderrResolveWs = beginReady.setupResolveWriteStreamPipe(
  ...
  _executeCommand(command, args, argsNoSplit, debug) {
    ...
    if (this._process.signalCode === "SIGTERM") {
      return Promise.reject(
        new Error("Could not connect to the exiftool process")
      );
    }

    const proc = debug === true ? process : this._process;
    return lib.executeCommand(
      proc,
      this._stdoutResolveWs,
      this._stderrResolveWs,
      command,
      args,
      argsNoSplit,
      this._encoding
