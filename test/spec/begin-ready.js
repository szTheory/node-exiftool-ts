...
const createBeginReadyMatchTransformStream =
  beginReady.createBeginReadyMatchTransformStream;
...
const brtsTestSuite = {
  ...
  createResolverWriteStream: {
    ...
    "should call resolve and delete entry from resolveMap on data": () => {
      ...
      return new Promise((resolve) => rws.on("finish", resolve)).then(() => {
    ...
    "should call next with an error when command number cannot be found": () => {
      ...
      return new Promise((resolve) => rws.on("error", resolve)).then((err) => {
        assert.equal(
          err.message,
          `Command with index ${commandNumber2} not found`
        );
