...
function assertDoesNotExist(file) {
  return makepromise(fs.stat, [file]).then(
    () => {
...
const readFromRsTestSuite = {
  ...
  "should reject if non-readable passed": () => {
    return executeWithRs("string", null, () => {}).then(
      () => {
  ...
  "should return execute function result": (ctx) => {
    ...
    return executeWithRs(rs, null, executeCommand).then((res) => {
  ...
  "should call executeCommand with an existing file": (ctx) => {
    ...
    const executeCommand = (_tempFile) => {
      tempFile = _tempFile;
      return assertExists(tempFile).then(
        () => {
          fileCreated = true;
        },
        (_error) => {
    ...
    return executeWithRs(rs, null, executeCommand).then(() => {
      assert(fileCreated);
  ...
  "should call executeCommand with args": (ctx) => {
    ...
    return executeWithRs(rs, testArgs, executeCommand).then(() => {
