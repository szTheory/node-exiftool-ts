...
function createCheckPids(forkPid, epPid, epChildPid, conhostPid) {
  ...
  const checkPids = () =>
    checkPid(arr).then((res) => {
      const fork = res.find((r) => r.pid === `${forkPid}`);
      const ep = res.find((r) => r.pid === `${epPid}`);
      const epChild = epChildPid
        ? res.find((r) => r.pid === `${epChildPid}`)
        : undefined;
      const conhost = conhostPid
        ? res.find((r) => r.pid === `${conhostPid}`)
        : undefined;
      const o = {
        fork,
        ep,
...
const findExiftoolChildAndConhost = (epPid, exiftoolDetached) => {
  if (!isWindows) {
    return Promise.reject(
      new Error("This function is only available on Windows")
    );
  }
  return checkPpid(epPid)
    ...
    .then((res) => {
      ...
      assert(
        conhost,
        "conhost.exe should have been started as child of exiftool"
      );
      const conhostPid = conhost.pid;
      const epChild = res.find((p) => /exiftool\.exe/.test(p.command));
      assert(
        epChild,
        "exiftool.exe should have been started as child of exiftool"
      );
...
const setup = (exiftoolDetached, ctx) => {
  ...
  return ctx
    ...
    .then((meta) => {
      ...
      if (isWindows) {
        return findExiftoolChildAndConhost(epPid, exiftoolDetached).then((r) =>
          Object.assign(res, r)
        );
      }
      return res;
...
if (isWindows) {
  Object.assign(DetachedTrueTestSuite, {
    ...
    "should not quit child process when fork exits with detached option (win)": createTestWin(
      true
    ),
  });
} else {
