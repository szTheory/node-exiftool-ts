...
const OptionsTestSuite = {
  ...
  "calls child_process.spawn with specified options": (ctx) => {
    ...
    return ctx.createOpen(options).then(() => {
      assert.equal(ctx.proc.args.options, options);
    });
  },
  "returns rejected promise when trying to open without readable stderr": (
    ctx
  ) => {
    const options = {
    ...
    return ctx.createOpen(options).then(
      ...
      (err) => {
        assert.equal(
          err.message,
          "Process was not spawned with a readable stderr, check stdio options."
        );
      }
    );
  },
  "returns rejected promise when trying to open without readable stdout": (
    ctx
  ) => {
    const options = {
