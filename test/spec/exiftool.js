...
const exiftoolTestSuite = {
  ...
  open: {
    "opens exiftool": (ctx) => {
      return ctx.createOpen().then((pid) => {
  ...
  close: {
    ...
    "completes remaining jobs": (ctx) => {
      return ctx.createOpen().then(() => {
        const p = ctx.ep.readMetadata(ctx.jpegFile).then((res) => {
          assert(Array.isArray(res.data));
          assert.equal(res.error, null);
          res.data.forEach(ctx.assertJpegMetadata);
        });
        const p2 = ctx.ep.readMetadata(ctx.jpegFile2).then((res) => {
          assert(Array.isArray(res.data));
          assert.equal(res.error, null);
          res.data.forEach(ctx.assertJpegMetadata);
        });
  ...
  readMetadata: {
    ...
    "reads metadata of files in a directory": (ctx) => {
      return ctx.initAndReadMetadata(ctx.folder).then((res) => {
        assert(Array.isArray(res.data));
        assert.equal(res.data.length, 5);
        res.data.forEach(ctx.assertJpegMetadata);
        assert.equal(
          res.error,
          `1 directories scanned${EOL}    5 image files read`
        );
      });
    },
    "returns null data for empty directory and info error": (ctx) => {
      return ctx.initAndReadMetadata(ctx.emptyFolder).then((res) => {
        assert.equal(res.data, null);
        assert.equal(
          res.error,
          `1 directories scanned${EOL}    0 image files read`
        );
      });
    },
    "allows to specify arguments": (ctx) => {
      return ctx
        .initAndReadMetadata(ctx.jpegFile, ["Orientation", "n"])
        .then((res) => {
          assert.equal(res.error, null);
          assert(Array.isArray(res.data));
          const expected = {
    ...
    "reads metadata of a file": (ctx) => {
      return ctx.initAndReadMetadata(ctx.jpegFile).then((res) => {
        assert.equal(res.error, null);
        assert(Array.isArray(res.data));
    ...
    "returns promise with null data and error when file not found": (ctx) => {
      return ctx.initAndReadMetadata(ctx.fileDoesNotExist).then((res) => {
        assert.equal(res.data, null);
    ...
    "works with simultaneous requests": (ctx) => {
      return ctx
        ...
        .then((res) => {
          ...
          assert.equal(
            res[1].error,
            `File not found: ${ctx.fileDoesNotExist2}`
          );
  ...
  writeMetadata: {
    "returns rejected promise when trying to execute when not open": (ctx) => {
      return ctx
        .create()
        .writeMetadata("/temp-file", { comment: "test-comment" }, [
          "overwrite_original",
        ])
        .then(() => {
    ...
    "should return rejected promise when data is not an object": (ctx) => {
      return ctx.initAndWriteMetadata("file_path").then(
        () => {
          throw new Error("writeMetadata should have resulted in error");
        },
        (err) => {
    ...
    "should write metadata": (ctx) => {
      ...
      return ctx
        .createTempFile()
        .then(() =>
          ctx.initAndWriteMetadata(ctx.tempFile, data, ["overwrite_original"])
        )
        .then((res) => {
          assert.equal(res.data, null);
